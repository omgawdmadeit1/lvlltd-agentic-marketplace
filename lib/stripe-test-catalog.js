...
const CHECKOUT_CANCEL_URL = "https://agentic.lvlltd.com/buy?checkout=cancel";
...
function checkoutSuccessUrl(env) {
  const source = env || process.env;
  const raw = String(source.CHECKOUT_SUCCESS_URL || "").trim();
  if (!raw) {
    return { ok: true, url: DEFAULT_CHECKOUT_SUCCESS_URL };
  }
  let parsed;
  try {
    parsed = new URL(raw);
    ...
    return {
      ok: false,
      error: "invalid_checkout_success_url",
      message:
        "CHECKOUT_SUCCESS_URL must be an https URL containing {CHECKOUT_SESSION_ID}.",
    };
  }
  if (parsed.protocol !== "https:" || !raw.includes(CHECKOUT_SESSION_PLACEHOLDER)) {
    return {
      ok: false,
      error: "invalid_checkout_success_url",
      message:
...
function packDownloadUrl(sessionId, env) {
  const resolved = checkoutSuccessUrl(env);
  const template = resolved.ok ? resolved.url : DEFAULT_CHECKOUT_SUCCESS_URL;
  const id = String(sessionId || "").trim();
  if (!id) return null;
  return template.split(CHECKOUT_SESSION_PLACEHOLDER).join(encodeURIComponent(id));
}
