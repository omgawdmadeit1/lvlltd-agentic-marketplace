...
const DEFAULT_PUBLIC_ORIGIN = "https://agentic.lvlltd.com";

/** Public origin for success/cancel/download links. Never derived from request headers. */
function deliveryOrigin(env) {
  const raw = String((env || process.env).DELIVERY_PUBLIC_ORIGIN || "")
    .trim()
    .replace(/\/+$/, "");
  return /^https?:\/\/[A-Za-z0-9.-]+(:\d+)?$/.test(raw) ? raw : DEFAULT_PUBLIC_ORIGIN;
}

/** Checkout success_url: the server-verified download page on this storefront. */
function checkoutSuccessUrl(env) {
  return `${deliveryOrigin(env)}/buy/success?session_id={CHECKOUT_SESSION_ID}`;
}

/** Download/order page for a known session id (null when no id). */
function successPageUrl(sessionId, env) {
  const id = String(sessionId || "").trim();
  if (!id) return null;
  return `${deliveryOrigin(env)}/buy/success?session_id=${encodeURIComponent(id)}`;
}
